        ...
        for room_num, room in enumerate(self.room_list):
            for b in range(room[2]):
                for c in range(room[3]):
                    #ajout d'objets / monstres supplémentaires aléatoirement:
                    random_number = random.random()
                    if random_number < 0.88 :
                        self.level[room[1] + c][room[0] + b] = 'floor'
                    elif random_number >= 0.88 and random_number < 0.90:
                        self.level[room[1] + c][room[0] + b] = 'protection' #ajout de protection 'V' au sol
                    elif random_number >= 0.90 and random_number < 0.97:
                        self.level[room[1] + c][room[0] + b] = 'money' #ajout de money 'o' au sol
                    else:
                        self.level[room[1] + c][room[0] + b] = 'PV' #ajout de PV 'A' au sol
        # paint corridors
    ...
    def gen_tiles_level(self):
        for row_num, row in enumerate(self.level):
            ...
            for col_num, col in enumerate(row):
                ...
                if col == 'protection':
                    tmp_tiles.append(self.tiles['protection'])
                if col == 'PV':
                    tmp_tiles.append(self.tiles['PV'])
